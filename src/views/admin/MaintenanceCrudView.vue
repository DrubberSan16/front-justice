    ...
    <div class="responsive-header mb-3">
      <div>
        ...
        <v-chip
          ...
          :aria-label="`Quitar el filtro por tipo ${equipmentTypeFilterLabel}`"
          @click:close="clearEquipmentTypeFilter"
        >
          {{ equipmentTypeFilterLabel }}
        </v-chip>
      </div>
...
/**
 * Filtro por tipo de equipo, gobernado por la URL.
 *
 * El menu cuelga un hijo por cada tipo y todos abren esta misma pantalla; lo
 * que los distingue es `?tipo=`. Vive en la URL a proposito: asi el enlace se
...
const equipmentTypeFilterLabel = computed(() => {
  if (!equipmentTypeFilterId.value) return "";
  const options = relationOptions.value.equipo_tipo_id ?? [];
  const match = options.find(
    (option) => String(option.value) === equipmentTypeFilterId.value,
  );
  return match ? String(match.title) : "Tipo seleccionado";
});
...
function isAutoCodeField(field: EnhancedMaintenanceField) {
...
function isMaterialField(field: EnhancedMaintenanceField) {
  return (
