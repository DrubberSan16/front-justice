...
function getProductOptions() { if (!documentForm.bodegaId) return []; return products.value.filter((product) => documentForm.tipo !== "SALIDA" || Number(stockByWarehouseProduct.value.get(`${documentForm.bodegaId}:${product.id}`)?.stock_actual || 0) > 0).map((product) => { const stock = stockByWarehouseProduct.value.get(`${documentForm.bodegaId}:${product.id}`); const stockLabel = stock ? ` · stock ${formatNumberForDisplay(stock.stock_actual)}` : ""; return { value: product.id, title: `${buildProductDisplayTitle(product)}${stockLabel}` }; }); }
