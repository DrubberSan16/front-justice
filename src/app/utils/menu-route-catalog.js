...
const availableViewFiles = new Set(Object.keys(frontViewModules).map((path) => path.replace(/\\/g, "/").replace(/^\.\.\/\.\.\//, "")));
export function normalizeMenuRouteKey(value) {
    return String(value ?? "")
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .replace(/\\/g, "/")
...
function resolveViewFile(route) {
    const raw = route.meta?.viewFile;
    if (typeof raw !== "string")
        return null;
    return raw.replace(/\\/g, "/").replace(/^src\//, "");
}
function buildAliases(route, viewFile) {
...
function isMenuAssignableRoute(route) {
...
export function getMenuRouteCatalog(router) {
    return router
        .getRoutes()
...
export function findMenuRouteByValue(router, urlComponent) {
    const target = normalizeMenuRouteKey(urlComponent);
    if (!target)
        return null;
    return (getMenuRouteCatalog(router).find((item) => item.aliases.includes(target)) ?? null);
}
export function coerceMenuComponentValue(router, urlComponent) {
    return findMenuRouteByValue(router, urlComponent)?.routeName ?? urlComponent;
}
export function resolveMenuRouteLocation(router, urlComponent) {
