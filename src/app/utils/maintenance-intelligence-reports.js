...
function normalizeRows(rows) {
    return rows.map((row) => Object.fromEntries(Object.entries(row).map(([key, value]) => [prettifyLabel(key), formatValue(value)])));
}
...
function inferColumnFormat(key) {
    ...
    if (/fecha|date/.test(normalized))
        return normalized.includes("hora") ? "datetime" : "date";
    if (/hora|hours|hrs/.test(normalized))
        return "hours";
    if (/costo|precio|subtotal|total|valorado/.test(normalized))
        return "currency";
    if (/cantidad|stock|promedio|indice|porcentaje|kw|galones|saldo|horometro|resultado|nivel/.test(normalized))
        return "number";
    return "text";
}
function inferColumnWidth(key, rows) {
    const headerLength = prettifyLabel(key).length;
    const sampleLength = Math.max(0, ...rows.slice(0, 30).map((row) => String(formatValue(row[key]) || "").length));
    return Math.min(40, Math.max(12, headerLength + 2, Math.ceil(sampleLength * 0.85)));
}
function resolveColumns(sheet, rows) {
...
function buildGroupedRows(rows, groupBy) {
    if (!groupBy?.length) {
...
export async function downloadReportPdf(report) {
    ...
    const generatedLabel = report.generatedAt
        ? new Date(report.generatedAt).toLocaleString()
        : new Date().toLocaleString();
    function drawPageHeader(title, subtitle, pageLabel) {
    ...
    for (const [index, sheet] of report.sheets.entries()) {
        ...
        autoTable(doc, {
            ...
            body: safeRows.map((row) => columns.map((column) => repairText(String(formatValue(row[column.key] ?? row[column.header]))))),
        });
...
export function buildProceduresReport(procedures) {
    ...
    const activityRows = procedures.flatMap((item) => (item.actividades ?? []).map((activity) => ({
...
export function buildLubricantReport(analyses) {
    ...
    const detailRows = analyses.flatMap((item) => (item.detalles ?? []).map((detail) => ({
    ...
    const alerts = analyses.filter((item) => String(item.estado_diagnostico || "").toUpperCase() === "ALERTA").length;
    return {
...
export function buildComponentsReport(components) {
    ...
    const inAlert = components.filter((item) => ["ALERTA", "CRITICO", "CRITICA", "POR CAMBIO"].includes(String(item.estado || "").toUpperCase())).length;
    return {
...
export function buildDailyReportsReport(reports) {
    ...
    const unitRows = reports.flatMap((item) => (item.unidades ?? []).map((unit) => ({
    ...
    const fuelRows = reports.flatMap((item) => (item.combustibles ?? []).map((fuel) => ({
        reporte_codigo: item.codigo,
        fecha_reporte: item.fecha_reporte ?? "",
        tanque: fuel.tanque ?? "",
    ...
    const componentRows = reports.flatMap((item) => (item.componentes ?? []).map((component) => ({
        reporte_codigo: item.codigo,
        fecha_reporte: item.fecha_reporte ?? "",
...
export function buildWeeklyScheduleReport(schedules) {
    ...
    const activityRows = schedules.flatMap((item) => (item.detalles ?? []).map((detail) => ({
