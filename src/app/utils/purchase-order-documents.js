...
function splitCurrency(value, currency = "USD") {
    return formatCurrency(value, currency).replace(/\$/g, "").trim();
}
export async function downloadPurchaseOrderPdf(order, userName = "Sistema") {
    ...
    doc.setFontSize(9);
    doc.text("GUAYAS / GUAYAQUIL / AV. DE LAS AMÉRICAS / JUSTICE COMPANY", marginX + 14, 62);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.text(`Bodega destino: ${repairText(order.bodega_label || "Bodega matriz")}`, rightX, 98, { align: "right" });
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    const obsLines = doc.splitTextToSize(repairText(order.observacion || order.condicion_pago || "Sin observación"), pageWidth - marginX * 2 - 24);
    ...
    const finalY = doc.lastAutoTable?.finalY ?? cursorY + 120;
