...
function slugifyCode(value: unknown) {
...
function sheetCellValue(sheet: XLSX.WorkSheet, rowIndex: number, columnIndex: number) {
  const cell = sheet[XLSX.utils.encode_cell({ r: rowIndex, c: columnIndex })];
  return cell?.v ?? null;
}

function sheetCellText(sheet: XLSX.WorkSheet, rowIndex: number, columnIndex: number) {
...
function findRowByLabel(sheet: XLSX.WorkSheet, label: string) {
...
function findTextNearLabel(sheet: XLSX.WorkSheet, labelRow: number, labelColumn: number) {
...
function resolveWorkbookEquipmentHint(workbook: XLSX.WorkBook, fileName: string) {
...
function resolveEquipment(
  ...
  const equipment =
    equipments.find((item) => slugifyCode(item.codigo) === normalizedHint) ||
  ...
  return {
    ...
    equipo_marca:
      String(equipment?.marca_nombre ?? "").trim() ||
...
function normalizeCompartment(sheetName: string) {
  const target = normalizeToken(sheetName);
  return (
    lubricantCompartments.find((item) => normalizeToken(item) === target) ??
    sheetName.trim().toUpperCase()
  );
}

function collectSampleColumns(sheet: XLSX.WorkSheet) {
...
export function parseLubricantWorkbook(
  ...
  const validSheets = workbook.SheetNames.filter((sheetName) =>
    lubricantCompartments.some(
      (compartment) => normalizeToken(compartment) === normalizeToken(sheetName),
    ),
  );
  ...
  const equipmentContext = resolveEquipment(
    equipmentHint,
    options.equipments ?? [],
    brandMap,
  );

  const analyses: AnyRow[] = [];
  ...
  for (const sheetName of validSheets) {
    ...
    const sampleRows = Object.fromEntries(
      Object.entries(SAMPLE_ROW_LABELS).map(([key, label]) => [key, findRowByLabel(sheet, label)]),
    ) as Record<keyof typeof SAMPLE_ROW_LABELS, number | null>;
