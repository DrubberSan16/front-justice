...
async function registerMovementAndKardex(args) {
    ...
    if (args.tipo === "INGRESO")
        movPayload.bodega_destino_id = args.bodegaId;
    if (args.tipo === "SALIDA")
        movPayload.bodega_origen_id = args.bodegaId;
...
export async function performManualMovement(payload) {
    const stocks = (await listAll("/kpi_inventory/stock-bodega"));
...
export async function bulkUpsertFromRows(rows, userName) {
