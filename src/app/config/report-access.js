...
const REPORT_ACCESS_KEY_SET = new Set(REPORT_ACCESS_OPTIONS.map((item) => item.value));
export function normalizeReportAccess(value) {
    if (!Array.isArray(value))
        return [];
    return [...new Set(value.map((item) => String(item || "").trim()).filter((item) => REPORT_ACCESS_KEY_SET.has(item)))];
}
export function hasReportAccess(allowedReports, reportKey) {
