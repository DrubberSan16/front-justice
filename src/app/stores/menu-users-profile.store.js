...
const ENDPOINTS = {
    ...
    byUser: (userId) => `/kpi_security/menu-users/by-user/${userId}?includeDeleted=true`,
    create: `/kpi_security/menu-users`,
    patch: (id) => `/kpi_security/menu-users/${id}`,
};
const defaultDraft = (menuId) => ({
    menuId,
    enabled: false,
    isReaded: false,
    isCreated: false,
    ...
    actions: {
        ...
        getDraft(menuId) {
            if (!this.drafts[menuId])
                this.drafts[menuId] = defaultDraft(menuId);
            return this.drafts[menuId];
        },
        // 1) Cargar perfil del usuario (para edición)
        async loadByUser(userId) {
            this.loading = true;
            this.error = null;
            try {
                this.drafts = {};
                this.original = {};
                const { data } = await api.get(ENDPOINTS.byUser(userId));
        ...
        setDraftsFromRoleMenus(menuRoles) {
            this.reset();
        ...
        async createProfileForUser(userId, createdBy) {
            this.loading = true;
            this.error = null;
            try {
